...
Object.defineProperty(window, 'weights', {
  get() {
    return profileManager.weights;
  },
  set(value) {
    profileManager.weights = value;
  },
});

Object.defineProperty(window, 'passThreshold', {
  get() {
    return profileManager.passThreshold;
  },
  set(value) {
    profileManager.passThreshold = value;
  },
});
...
Object.defineProperty(window, 'classes', {
  ...
  set(value) {
    classManager.classes = value;
  },
});

Object.defineProperty(window, 'currentClass', {
  get() {
    return classManager.currentClass;
  },
  set(value) {
    classManager.currentClass = value;
  },
});

Object.defineProperty(window, 'classListData', {
  get() {
    return classManager.classListData;
  },
  set(value) {
    classManager.classListData = value;
  },
});
...
async function initializeApp() {
  ...
  try {
    ...
    // Only initialize profiles and classes if authenticated
    if (window.AuthModule && !window.AuthModule.isAuthenticated) {
      window.AuthModule.showLoginPage();
      return;
    }

    // Initialize profiles and classes in parallel for better performance
    await Promise.all([profileManager.init(), classManager.init()]);
