...
async function renderLayoutPage(
  ...
  const body = ejs.render(
    template,
    { ...pageData, assetPath: app.locals.assetPath, assetVersion: app.locals.assetVersion },
    { filename: viewPath }
  );
...
function serializeProfile(profileDoc = {}) {
  const weightsMap =
    profileDoc.weights instanceof Map
      ? profileDoc.weights
...
function serializeClass(classDoc = {}) {
  ...
  if (gradesDoc && typeof gradesDoc === 'object') {
    const rawGradeStudents = (() => {
      ...
      if (
        gradesDoc.students &&
        typeof gradesDoc.students === 'object' &&
        !Array.isArray(gradesDoc.students)
      ) {
      ...
      if (
        !Object.prototype.hasOwnProperty.call(gradesDoc, 'students') &&
        !Array.isArray(gradesDoc)
      ) {
    ...
    const studentEntries =
      rawGradeStudents instanceof Map
    ...
    normalizedGrades = {
      ...
      students: studentEntries.reduce((acc, [key, value]) => {
        ...
        acc[mssv] = value && typeof value === 'object' && !Array.isArray(value) ? { ...value } : {};
        return acc;
      }, {}),
    };
  }
...
function ensureLecturerAccess(req, res) {
  if (!req.user) {
    if (req.path === '/grade-check' && req.query.login === 'required') {
      return true;
    }
    res.redirect('/grade-check?login=required');
    return false;
  }
...
app.get('/', (req, res) => {
  const queryString = Object.keys(req.query).length
    ...
    : '';
  res.redirect(`/grade-check${queryString}`);
});
  ...
  try {
    ...
    if (userId) {
      ...
      if (profilesResult.status === 'fulfilled') {
        ...
        console.warn(
          'Unable to preload profiles for /classes:',
          profilesResult.reason?.message || profilesResult.reason
        );
      }
      ...
      if (classesResult.status === 'fulfilled') {
        ...
        console.warn(
          'Unable to preload classes for /classes:',
          classesResult.reason?.message || classesResult.reason
        );
      }
    }
    ...
    await renderLayoutPage(req, res, 'classes', {
      ...
      initialData:
        Object.keys(profilesById).length || Object.keys(classesById).length
          ? {
...
app.get('/branding', optionalAuth, async (req, res, next) => {
  if (!req.user) {
    return res.redirect('/grade-check?login=required');
  }
...
app.get('/template', optionalAuth, async (req, res, next) => {
  try {
    ...
    if (profilesResult.status === 'rejected') {
      console.warn(
        'Unable to load profiles for template page:',
        profilesResult.reason?.message || profilesResult.reason
      );
    }
    ...
    if (classesResult.status === 'rejected') {
      console.warn(
        'Unable to load classes for template page:',
        classesResult.reason?.message || classesResult.reason
      );
    }
    ...
    const selectedProfileId =
      profiles.find(profile => profile.isDefault)?.profileId || profiles[0]?.profileId || '';
